...
Note: if two batteries of the same type and number of cells are used as power sources, they will be selected alternatively <sup><a href="#note9">9</a></sup> and will discharge in parallel.
...
However, an helicopter could still be landed in autorotaion after a failure of the main power source and the RX Dual Power can be configured with a STBY battery just like an airplane with electric motor.
