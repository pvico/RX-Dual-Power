...
As long as MAIN PWR is above its minimum voltage (according to its type), the MCU selects MAIN PWR and forces STBY PWR off <sup><a href="#note5">5</a></sup>.

When MAIN PWR is *below its minimum voltage* <sup><a href="#note6">6</a></sup> - but **not** disconnected or in short-circuit - the MCU selects STBY PWR and forces MAIN PWR off <sup><a href="#note7">7</a></sup>.
...
The MCU lets the LTC4412's control the power source <sup><a href="#note8">8</a></sup>. 

The LTC4412's select the source with the highest voltage and isolate the other one from the sytem.

Note: if two batteries of the same type are used as power source, they will be selected alternatively <sup><a href="#note9">9</a></sup> and will discharge in parallel.
...
<sub><sup id="note1">1</sup> Battery Eliminator Circuit, a device reducing the battery voltage to a level acceptable by the radio system. Most of the time, it is a buck converter, a switching step-down DC-DC converter.</sub><br/>
...
<sub><sup id="note8">8</sup> Both CTL1 and CTL2 lines are released (MCU pins set to high impedance).</sub><br/>
<sub><sup id="note9">9</sup> Whichever one is 20mV above the other will power the model.</sub><br/>
